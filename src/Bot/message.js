...
module.exports = async (message, client) => {
    ...
    if (
        ...
        message.channel.id == config.permissions.channel
    ) {
        if (
            message.embeds[0] &&
        ...
        if (
            ...
            message.embeds[0].author.name.includes('posting session')
        ) {
            await sleep(randomInt(1000, 2500));
        ...
        if (
            message.embeds[0] &&
            message.embeds[0].title &&
            message.embeds[0].title.includes('balance')
        ) {
            await sleep(randomInt(1000, 2500));

            if (message.embeds[0].description.includes('100.0%')) {
                client.users.cache
                    .get(config.permissions.main_account)
                    .dmChannel.send('**Hi!** My bank balance is full.');
            }
    ...
    if (!message.content.toLowerCase().startsWith(config.permissions.prefix))
        return;
